...
def create_employee_table(cur, conn):
    cur.execute('create table if not exists employees (employee_id INTEGER PRIMARY KEY, first_name TEXT, job_id INTEGER, hire_date DATE, salary NUMBER)')
    conn.commit
...
def add_employee(filename, cur, conn):
    ...
    for item in data:
        ...
        h_date = item['hire_date']
        j_id = item['job_id']
        income = item['salary']

        cur.execute('insert or ignore into employees(employee_i,first_name,last_name,hire_date,job_id,salary)') (emp_id,first_n,last_n, h_date, j_id,income)
        conn.commit
